...
#              bash pattern-HIVp.sh 1NH0_ref.pdb pattern01
#
...
#
#     Example:   sh pattern-HIVp.sh 1NH0_ref.pdb pattern01
#                    keeps intermediary files of the mutation pattern applied to 1NH0_ref.pdb
...
then
	...
	echo
	echo "    Examples:"
	echo "		./pattern-HIVp.sh 1NH0_ref pattern01"
	echo "		sh pattern-HIVp.sh 1NH0_ref.pdb pattern01"
	echo
    exit 0
fi
...
do
	...
	mv mut_${opdb%%.pdb}_${mutation}/${opdb%%.pdb}_${mutation}.pdb ./${opdb%_*.pdb}_${mutation}.pdb
	rm -r mut_${opdb%%.pdb}_${mutation}/
	npdb="${opdb%_*.pdb}_${mutation}.pdb"
done < ../$patf
cd ..
echo "All done, "$USER"! A new PDB with a mutation pattern was created in "$patfoo"."
